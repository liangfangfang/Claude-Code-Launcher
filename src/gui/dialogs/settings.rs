...
pub fn view(state: &State) -> Element<'_, Message> {
    ...
    let tab_btns = row![
        tab_button("备份管理", Tab::Backup, state.active_tab),
        tab_button("模板管理", Tab::Templates, state.active_tab),
        tab_button("全局配置", Tab::Global, state.active_tab),
    ]
    .spacing(4);

    let tab_content = match state.active_tab {
        Tab::Backup => backup_tab(state),
        Tab::Templates => templates_tab(state),
        Tab::Global => global_tab(state),
    };
...
fn backup_tab(state: &State) -> Element<'_, Message> {
    let status_text = format!(
        "settings.json 状态：{}\n备份文件状态：{}",
        state.backup_status.status_text(),
        if state.backup_status.backup_exists {
            "有"
        } else {
            "无"
        }
    ...
    let backup_btn = if state.backup_status.backup_exists {
        button(text("备份（已有备份）").size(13)).style(theme::toolbar_btn_style())
    } else {
        button(text("备份").size(13))
            .on_press(Message::BackupClicked)
            .style(theme::accent_btn_style())
    };
    let disable_btn = if !state.backup_status.settings_exists {
        button(text("失效（无设置文件）").size(13)).style(theme::toolbar_btn_style())
    } else {
    ...
    let restore_btn = if !state.backup_status.backup_exists {
        button(text("恢复（无备份文件）").size(13)).style(theme::toolbar_btn_style())
    } else {
    ...
    let btns = row![backup_btn, disable_btn, restore_btn].spacing(10);
    ...
    let mut col = column![status, btns].spacing(15);
    if let Some(m) = msg {
        col = col.push(m);
    }
...
fn global_tab(state: &State) -> Element<'_, Message> {
    // 注入面板
    ...
    // 保存按钮
    let save_btn = button(text("保存").size(13))
